...
void SubmatrixCalculator::calculateCostSubmatrix(string strLeft, string strTop,
  ...
  for (int i = 1; i <= this->dimension; i++) {
    for (int j = 1; j <= this->dimension; j++) {
      ...
      // insert
      int alternative = lastSubV[i][j - 1] +
                        this->insertCost * (strTop[j - 1] != blankCharacter);
      ...
      // delete
      alternative = lastSubV[i - 1][j] +
                    this->deleteCost * (strLeft[i - 1] != blankCharacter);
      if (lastSubV[i][j] > alternative) {
        lastSubV[i][j] = alternative;
        lastSubH[i][j] = 1;
      }
...
inline void SubmatrixCalculator::calculateSubmatrix(string strLeft,
  ...
  for (int i = 1; i <= this->dimension; i++) {
    for (int j = 1; j <= this->dimension; j++) {
      ...
      lastSubV[i][j] = mmin(R - lastH, delCost, insCost + lastV - lastH);
      lastSubH[i][j] = mmin(R - lastV, insCost, delCost + lastH - lastV);
      /*  if (strLeft == "A-" && strTop == "G-" && stepLeft == "21" && stepTop
        == "21"){
            cout << "lastv " << lastV << " lasth " << lastH << " R " << R <<
        endl;
            cout << R - lastH << " " << this->deleteCost << " " <<
        this->insertCost + lastV - lastH << endl;
            cout << R - lastV << " " << this->insertCost << " " <<
        this->deleteCost + lastH - lastV << endl;
        }*/
    }
  } /*
   if (strLeft == "A-" && strTop == "G-" && stepLeft == "21" && stepTop ==
   "21"){
       ...
       cout << "left " << strLeft << " top " << strTop << " stepleft "
            << stepsToPrettyString(stepLeft) << " steptop " <<
